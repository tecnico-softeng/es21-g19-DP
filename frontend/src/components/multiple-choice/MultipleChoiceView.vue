  ...
  <ul>
    <li v-for="option in questionDetails.options" :key="option.id">
      <span
        ...
        v-html="
          convertMarkDown(
            studentAnswered(option.id) + '**[★]** ' + option.content + ' Priority: ' + option.priority
          )
        "
      ...
      <span
        v-else
        v-html="convertMarkDown(studentAnswered(option.id) + option.content + ' Priority: ' + option.priority)"
      />
    </li>
  </ul>
</template>
